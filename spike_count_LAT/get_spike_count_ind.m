function [spkcount] = get_spike_count_ind(Ts, cutTrials, cellnb, incode,  ind, win)
...
for i=1:trialcnt
    if ~isnan(ind(i))
       spkcount(ind(i),1)=length(find(spktimes>win(1)+evt(i) & spktimes<win(2)+evt(i)))/(win(2)-win(1));
    end
end

for i=1:size(spkcount,1)-100
    if sum(spkcount(i:i+99))==0
        spkcount(i:end)=NaN;
        break
    end
end
