...
for ti=1:length(trials)
    ...
    tempTrials(ti).Timing.NEV.REPONSE_ON_Sec = NaN;
    ...
    if all(parse(ti,:) > 0)
        ...
        trialStageSegment.encode = zeros(1,trials(ti).Behavior.StageNo);
         for si=1:trials(ti).Behavior.StageNo
            ...
            elseif si == 2, trialStageSegment.encode(si) = specs.Encodes.TARGET_ON;
            elseif si == 3, trialStageSegment.encode(si) = specs.Encodes.HOLD_TARGET_ON;
            elseif si == 4, trialStageSegment.encode(si) = specs.Encodes.GIVE_REWARD;
            end
        end
        ...
        while si <= trials(ti).Behavior.StageNo
            ...
            if segmentIndex>length(trialSegment.encode) || si>length(trialStageSegment.encode)
                ...
                if trials(ti).Behavior.StageNo==4 && trialSegment.encode(segmentIndex) == specs.Encodes.FIX_RELEASED
                    RESPONSE_ON_Sec=trialSegment.timeStampSec(segmentIndex);
                end                
        ...
        tempTrials(ti).Timing.NEV.RESPONSE_ON_Sec=RESPONSE_ON_Sec;
    end
